...
public class UsingList {
	...
	public static void main(String[] args) {
		...
		int skip = list.stream().skip(3).reduce(0, bi);
		System.out.println(skip);
		
		//allMatch, anyMatch example
		boolean  b = list.stream().allMatch(num -> num >2);
		System.out.println(b);
		
		boolean  b2 = list.stream().anyMatch(num -> num >2);
		...
		//boxing with stream
		List<Integer> afterBoxing = 
				IntStream.rangeClosed(1, 50) //primitive type
				.boxed() //wraperclass type
				.collect(Collectors.toList());
		System.out.println(afterBoxing);
		
		//unboxing with stream
		List<Integer> nums = Arrays.asList(1,3,6,8,9);
		int sumRes = nums.stream().mapToInt(Integer::intValue).sum();
		System.out.println(sumRes);
		
		double doubleRes = IntStream.rangeClosed(1, 50)  //primitive type
				.mapToDouble(x -> x) //wraper type
				//or
				//.mapToObject(x -> x)
				.sum();
...
public static void convertingListToStream() {
	...
	public static void removeDuplicateItemsfromList() {
		//some dummy data
		List<Integer> numList = Arrays.asList(1, 5, 7, 8, 9, 3, 1, 5);
		List<String> namesList = Arrays.asList("punya", "partha", "punya", "punya", "niraj");
	...
	public static void sortingTheList() {
		...
		//some dummy data
		List<Integer> numList = Arrays.asList(10,-7,108,17);
		List<String> namesList = Arrays.asList("punya","kishore","sai","aswini");
		List<User> usersList = Arrays.asList(new User(19d),new User(67d),new User(50d),new User(91d));
		...
		List<String> result = namesList.stream()
											.sorted(Comparator.comparing(name -> name.toString()))
											.collect(Collectors.toList());
		System.out.println(result);
		
		//sorting in desc
		List<String> result2 = namesList.stream()
											.sorted(Comparator.comparing(name -> name.toString())
		...
		//usersList.sort((User user1, User user2) -> user1.getScore() - user2.getScore());
		...
		//sorting in asc
		List<User> sortedUsersInAsc = usersList
								        .stream()
		...
		System.out.println();
		List<User> sortedUsersInDesc2 = usersList
									        .stream()
