...
public class CustomComparatorDemo {
	...
	public static void sortNames() {
		...
		Collections.sort(namesList, comparator1);
		System.out.println(namesList);
	...
	public static void sortEmpByName() {
		...
		List<Employee> employeeList = Arrays.asList(new Employee(1,"Shifoo","acc1","IT",25, 150000d),	new Employee(2,"Punya","acc2","IT",40, 250000d));
		List<Person> personList = Arrays.asList(new Person("Tom Jones", "abc"), new Person("Harry Major", "bcw"),new Person("Harry Major", "mnf"));

		// only gathering the employee names
		...
		// Comparator order reversal using reversed() method
		Comparator<Employee> empNameComparator7 = Comparator.comparing(Employee::getName).reversed();
		...
		Collections.sort(employeeList, new MyCustomComparator());
		employeeList.forEach(System.out::println);

		// ex-03 - Comparator creation using static method comparing()
		Comparator<Employee> empNameComparator2 = Comparator.comparing(Employee::getName)
				.thenComparing(Employee::getAge);
		Collections.sort(employeeList, empNameComparator2);
		employeeList.forEach(System.out::println);

		// ex-04 , using jdk8 Defining a Comparator using lambda expression
		Comparator<Employee> empNameComparator4 = (Employee emp1, Employee emp2) -> {
